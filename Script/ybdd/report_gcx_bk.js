...
function setGXCReportSize(reportId, width, height) {
    ...
    var margin_right = 0;
    var bjTable_height = 142;//$("#bjTable").height();
...
function get_option_bj_gcx(axis_sw_max, axis_sw_min, axis_ll_max, axis_ll_min, intervalX, strFormatX, _stnm) {
    ...
    for (i = 0; i < arr_legend.length; i++) {
        ...
        if ((key_name.indexOf("规则调度") > -1) || (key_name.indexOf("现状调度") > -1)) {
 ...
 * 显示窗口
 */
function show_report_gcx(t_range, _stcd, _stime, _etime, _userId, _stnm, _sttp, t_day, t_plan) {
    $("#span_loading").html("查询调度方案过程线对比信息中...");
    $(".loading-layer").show();
    $(".gxc-bj-title").html(_stnm + "过程线分析比较");


     var selectModes = getModel();
    var _plusType = Number($('input[type=radio][name=fdxs_model]:checked').val());
    var _rainPlus = $('#dsz').val() == "" ? 0 : Number($('#dsz').val());
    var _hisStormId = $('#selectLSQJ').val() == null ? -1 : Number($('#selectLSQJ').val());
    ...
    $.ajax({
        ...
        url: url,
        data: info,
        //url: "panelNew.aspx/get_DisPatchRule_Info",
        //data: "{'info':'" + info + "'}",
   ...
   /* if (_sttp == "XX" || _sttp == "DD") {
        ...
        $.ajax({
            ...
            dataType: 'json',
            async: false,
            url: "panelNew.aspx/get_XXHQ_DiffGcx",
            data: "{'info':'" + info + "'}",
            success: function (data) {
                tools.loadinghide(false);
                if (data == null || data == "") {
        ...
        $.ajax({
            ...
            dataType: 'json',
            async: false,
            url: "panelNew.aspx/get_Gcx_DisPatch_Dbfx",
            data: "{'info':'" + info + "'}",
            success: function (data) {
                tools.loadinghide(false);
...
function getBjTableData(_stcd, _stime, _etime, _userId, _day) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        async: false,
        url: "PanelNew.aspx/get_SingleStcdScheMaxZQ",
        data: "{'info':'" + info + "'}",
        success: function (data) {
            if (data.d == null || data.d == "") {
                tools.loadinghide(false);
