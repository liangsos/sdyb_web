...
function getModel() {
    ...
    $("input[name='checkModel']:checkbox").each(function () {
        if ($(this).is(":checked")) {
            if (res == "") {
                res += $(this).attr('value');
            } else {
            ...
            if (selectModelsName == null) {
                selectModelsName = $(this).attr("modelName");
            } else {
                selectModelsName += ',' + $(this).attr("modelName");
            }
        }
 ...
 */
$('#bar_rlt,#bar_tckz,#bar_tl,#bar_clearCache,#bar_yscz,#bar_sdl,#bar_hsyj').click(function (e) {
    var obj = $("#" + e.target.id);
 ...
 * 右侧工具栏鼠标移出事件，隐藏提示文字
 */
$('#bar_rlt,#bar_tckz,#bar_tl,#bar_clearCache,#bar_yscz,#bar_sdl').mouseout(function (e) {
 ...
 * 右侧工具栏鼠标移入事件，显示提示文字
 */
$('#bar_rlt,#bar_tckz,#bar_tl,#bar_clearCache,#bar_yscz,#bar_sdl').mouseover(function (e) {
...
function yb_yscz() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.code != 200) {
                tools.loadinghide(false);
                tools.show_message_success(data.message);
            } else {
                tools.loadinghide(true);
                tools.show_message_error_const(data.message);
                return;
