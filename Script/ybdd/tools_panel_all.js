 ...
 */
var Tools_Panel_All = function () {
    //面板初始化
    this.panelInit();
};

"use strict";
Tools_Panel_All.prototype = {
    //接口路径
    ApiUrlPath: "",
    //版本号
    version: "1.0.0",
    ...
    panelInit: function () {
        var that = this;
        //判断高度是否超出
        if (panelConfig_all.panel_default_height > $(document).height()) {
            panelConfig_all.panel_default_height = $(document).height();
        }
        that.setPanelSize(panelConfig_all.panel_default_width, panelConfig_all.panel_default_height, false);
        //echarts实例-过程线
        that.chartGcx = echarts.init($("#proLineBody_All")[0]);
        //echarts实例-日雨量柱状图
        that.chartRyl = echarts.init($("#rainInfoBody_All")[0]);
        //echarts实例-时段雨量柱状图
        that.chartSdyl = echarts.init($("#rainInfoHourBody_All")[0]);
        //echarts实例-断面信息
        that.chartDm = echarts.init($("#dmInfoBody_All")[0]);
        //面板可拖动处理
        ...
        $(that.parentId).find('.icon-max').click(function () {
            if ($(this).attr("data-type") == "max") {
                $(that.parentId).css({
                    "top": "50%",
                    "left": "50%",
                    "margin-left": "-" + panelConfig_all.panel_default_width / 2 + "px",
                    "margin-top": "-" + (that.show_type == "1" ? panelConfig_all.panel_default_height : panelConfig_all.panel_default_height2 + panelConfig_all.table_height_yb) / 2 + "px"
                });
                that.panel_actual_width = panelConfig_all.panel_default_width;
                that.panel_actual_height = that.show_type == "1" ? panelConfig_all.panel_default_height : panelConfig_all.panel_default_height2 + panelConfig_all.table_height_yb;
                //设置水情面板相关宽高
        ...
        $(that.parentId + " .nav a").click(function (e) {
            ...
            var otb = $(id + ' .dataTables_scroll table').DataTable();
            if (otb != undefined)
                otb.columns.adjust().draw();
         ...
         * 基本信息TAB点击事件
         **/
        $("#linkStationInfo_All").click(function () {
         ...
         * 过程线TAB点击事件
         **/
        $("#linkProLine_All").click(function () {
            if (!that.is_show_gcx) {
                if (that.show_type == "1") {
         ...
         * 降雨信息TAB点击事件
         **/
        $("#linkRainInfo_All").click(function () {
            if (!that.is_show_jyxx) {
                $("#btnSearchRain_All").click();
        ...
        * 降雨信息查询按钮
        **/
        $("#btnSearchRain_All").click(function () {
            $("#rainInfoBody_All").show();
            $("#rainInfoHourBody_All").hide();
            $("#btnReturnRyl_All").hide();
        ...
        * 返回日雨量-按钮
        **/
        $("#btnReturnRyl_All").click(function () {
            $("#rainInfoBody_All").show();
            $("#rainInfoHourBody_All").hide();
            $(this).hide();
        });
        ...
        $(this.parentId + " .tableHd," + this.parentId + " .tableSk," + this.parentId + " .tableHp," + this.parentId + " .tableXx").on("click", ".link-stcd", function () {
            ...
            $(this).addClass("hover");
            //当前标签页内容更新
            $(".tb_body_all li.active a:eq(0)").click();
        });
        ...
        $(that.parentId + " .tj_body_all li a").click(function (e) {
            ...
            if ($(this).attr("id") == "linkTj_Xx_All") {
                $("#linkDmInfo_All").parent().hide();
                $("#linkRainInfo_All").parent().hide();
                $("#linkStationInfo_All").parent().hide();
                $("#linkRankList").parent().hide();
            } else {
                $("#linkDmInfo_All").parent().show();
                $("#linkRainInfo_All").parent().show();
                $("#linkStationInfo_All").parent().show();
                $("#linkRankList").parent().show();
            }
            if ($(this).attr("id") == "linkTj_Kzztz_All") {//add by chl 2019-11-15
                that.getKzztzTableInfo();
            }
            //切换到过程线，切换到第一个
            if (that.isinitData) {
                var flag = $(this).attr("id").split("_")[1];
                $("#table" + flag + "_tj_dd tbody tr:eq(0)").click();;

                if ($(this).attr("id") == "linkTj_Xx_All") {
                    $("#linkProLine_All").click();
                }
                ...
                var flag = $(this).attr("id").split("_")[1];
                var zyg_index = 0;
                if ($(this).attr("id") == "linkTj_Hd_All") {
                    $('#tableHd_tj_dd tr').each(function (index, item) {                   // 遍历 tr
                        if ($(this).attr("_stcd") == "50103000")
                            zyg_index = index;
                    });
                    $("#tableHd_tj_dd tbody tr:eq(" + (zyg_index - 1) + ")").click();
                    $("#linkProLine_All").click();
                }
                else
                    $("#table" + flag + "_tj_dd tbody tr:eq(0)").click();;
        ...
        if ($(".ybsw-modify").is(":visible")) {
            ...
            if ((type == 1) || (type == 3)) {
                $.each(that.obj_data_gcx.ybsw, function (index, value) {
                    that.obj_yb_graphic.arr_graphic[index].position = that.chartGcx.convertToPixel({ xAxisIndex: 0, yAxisIndex: 0 }, that.obj_data_gcx.ybsw[index].value);
                });
            }
            if ((type == 2) || (type == 3)) {
                if (that.show_sttp == "RR") {
                    $.each(that.obj_data_gcx.ll_in, function (index, value) {
                        that.obj_yb_graphic.arr_graphic[that.obj_data_gcx.ybsw.length + index].position = that.chartGcx.convertToPixel({ xAxisIndex: 0, yAxisIndex: 1 }, that.obj_data_gcx.ll_in[index].value);
                    });
                }
                else {
                    $.each(that.obj_data_gcx.ybll, function (index, value) {
                        that.obj_yb_graphic.arr_graphic[that.obj_data_gcx.ybsw.length + index].position = that.chartGcx.convertToPixel({ xAxisIndex: 0, yAxisIndex: 1 }, that.obj_data_gcx.ybll[index].value);
                    });
                }
            }
            that.updateChartGcxGraphic();
    ...
    change_table_jh_input: function () {
        ...
        var that = this;
        $("#contProLine_All .tableSqxx").on("input propertychange", "input", function () {
            //强制改为数字
  ...
  **/
    ...
    create_table_kzztz: function (json) {
        ...
        $("#kzztz_table_All thead").html("");
        $("#kzztz_table_All tbody").html("");
        $("#kzztz_table_All thead").append(html_th.toString());
        $("#kzztz_table_All tbody").append(html_td.toString());
    },
    /**
     * 展示面板
    ...
    panelShow: function (_stcd, _stnm, _time, _sttp, _btime) {
        ...
        if (that.show_type == "3") {
            ...
            $("#linkTj_Xx_All").parent().show();
            $("#linkTj_Kzztz_All").parent().show();//2019-11-15 add by chl
        } else {
            $("#data_out").css({
                "display": "block"
            });
            $("#linkTj_Xx_All").parent().hide();
            $("#linkTj_Kzztz_All").parent().hide();//2019-11-15 add by chl
        }
        $("#contProLine_All .sidebar-right,#contProLine_All .sidebar-control-right").hide();
        $("#contProLine_All .field_yb").hide();
        ...
        $(this.parentId).resizable({
            ...
            maxHeight: null,
            maxWidth: null,
            minHeight: panelConfig_all.panel_default_height,
            minWidth: panelConfig_all.panel_default_width,
    ...
    create_table_jbxx: function () {
        var that = this;
        //先清空表格 
        $("#contStatinInfo_All .table-jbxx tr").remove();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#contStatinInfo_All .table-jbxx").append(html_empty.toString());
            },
            error: function (errorMsg) {
    ...
    create_table_jbxx_dd: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#contStatinInfo_All .table-jbxx").append(_html);
            },
            error: function (errorMsg) {
                tools.show_message_error("获取基本信息失败!");
                return;
    ...
    create_table_ybtj: function () {
        ...
        var day = selectDays;
        //先清空数据
        $('#tableHd_tj_dd tbody').empty();
        $('#tableHd_tj_dd thead').empty();
        $('.dataTables_scrollBody .tableHd thead').empty();
        $('#tableSk_tj_dd tbody').empty();
        $('#tableSk_tj_dd thead').empty();
        $('.dataTables_scrollBody .tableSk thead').empty();
        $('#tableHp_tj_dd tbody').empty();
        $('#tableHp_tj_dd thead').empty();
        ...
        var callBack = function () {
            ...
            jsonTj = that.ddTj_Data.data;
            
            if (jsonTj == null)
                return;

            if ((jsonTj.hasOwnProperty("dataXx") && jsonTj.dataXx != null)) {
                if ($('#tableXx_tj_dd').hasClass('dataTable')) {
                    var oldTable = $('#tableXx_tj_dd').dataTable();
            ...
            if (that.show_type == "3") {
                //河道下标 0:站名;1:最大流量;2:最高水位;3:最高水位时间;4:超警戒水位(流量);5:警戒水位;6:保证水位;7:保证流量
                if (jsonTj.dataHd != null) {
                    $.each(jsonTj.dataHd, function (index, item) {
                        if (index == 0) {//列名
                            _html_hd_thead = "<tr>";
                            $.each(item, function (i, data) {
                                if (i <= item.length - 3)
                                    _html_hd_thead += "<th>" + data + "</th>"
                            })
                            _html_hd_thead += "</tr>";
                        } else {
                            var font_start = "";
                            var font_end = "";
                            if (Number(item[2]) > Number(item[6])) {//下标1：最高水位；下标6：保证水位
                                font_start = "<font color='Red'><strong>"
                                font_end = "</strong></font>";
                            } else if (Number(item[2]) > Number(item[5])) {//下标1：最高水位；下标5：警戒水位
                                font_start = "<font color='Blue'><strong>"
                                font_end = "</strong></font>";
                            }
                            _html_hd += "<tr class='link-stcd' _stcd='" + item[item.length - 2] + "' _stnm='" + item[0] + "' _sttp='" + item[item.length - 1] + "'><td>" + item[0] + "</td><td>" + item[1] + "</td><td>" + font_start + item[2] + font_end + "</td><td>" + item[3] + "</td><td>" + item[4] + "</td><td>" + item[5] + "</td><td>" + item[6] + "</td><td>" + item[7] + "</td></tr>";

                        }
                    });
                }
                if (jsonTj.dataSk != null) {
                    //水库下标 0:站名;1:当前水位;2:当前出库流量;3:预报最高水位;4:超汛限水位;5:汛限水位;6:设计水位;7:校核水位;8:全陪高程;9:移民高程;10:历史最高水位
                    $.each(jsonTj.dataSk, function (index, item) {
                        if (index == 0) {//列名
                            _html_sk_thead = "<tr>";
                            $.each(item, function (i, data) {
                                if ( i <= item.length - 5) { 
                                    _html_sk_thead += "<th>" + data + "</th>"
                                } else if (i >= item.length - 2) { //——新增最大下泄放在最后一列 2022.6.7 _——新增峰现时间 2022.6.8
                                    _html_sk_thead += "<th>" + data + "</th>"
                                }
                            })
                            _html_sk_thead += "</tr>";
                            ...
                            _html_sk += "<tr class='link-stcd' _stcd='" + item[item.length - 4] + "' _stnm='" + item[0] + "' _sttp='" + item[item.length - 3] + "'><td>" + item[0] + "</td><td>" + font_start + item[1] + font_end + "</td><td>" + item[2] + "</td><td>" + font_start + item[3] + font_end + "</td><td>" + item[4] + "</td><td>" + item[5] + "</td><td>" + item[6] + "</td><td>" + item[7] + "</td><td>" + item[8] + "</td><td>" + item[9] + "</td><td>" + item[10] + "</td><td>" + item[item.length - 2] +"</td><td>" + item[item.length - 1] + "</td></tr>";
                            ...
                            if (item[item.length - 4] == that.show_stcd) {
                                existType = "sk";
                                existIndex = index;
                ...
                if (jsonTj.hasOwnProperty("dataHp")) {
                    $("#linkTj_Hp_All").parent().show();
                    $("#linkTj_Hp_All").css({
                        "display": "block"
                    })
                    if (jsonTj.dataHp != null) {
                        $("#linkTj_Hp_All").parent().show();
                        $("#linkTj_Hp_All").css({
                            "display": "block"
                        })
                        $.each(jsonTj.dataHp, function (index, item) {
                            //湖泊
                            if (index == 0) {//列名
                                _html_hp_thead = "<tr>";
                                $.each(item, function (i, data) {
                                     if ( i <= item.length - 5) { 
                                         _html_hp_thead += "<th>" + data + "</th>"
                                     } else if (i >= item.length - 2) { //——新增最大下泄放在最后一列 2022.6.7 _——新增峰现时间 2022.6.8
                                         _html_sk_thead += "<th>" + data + "</th>"
                                     }
                                })
                                ...
                                var font_start = "";
                                var font_end = "";
                                if (Number(item[1]) > Number(item[7])) {
                                    font_start = "<font color='Red'>"
                                    font_end = "</font>";
                                }
                                else if (Number(item[1]) > Number(item[6])) {
                                    font_start = "<font color='Blue'>"
                                    font_end = "</font>";
                                }

                                _html_hp += "<tr class='link-stcd' _stcd='" + item[item.length - 4] + "' _stnm='" + item[0] + "' _sttp='" + item[item.length - 3] + "'><td>" + item[0] + "</td><td>" + font_start + item[1] + font_end + "</td><td>" + item[2] + "</td><td>" + item[3] + "</td><td>" + item[4] + "</td><td>" + item[5] + "</td><td>" + item[6] + "</td><td>" + item[7] + "</td><td>" + item[8] + "</td><td>" + item[9] + "</td><td>" + item[10] + "</td><td>" + item[11] + "</td><td>" + item[item.length - 2]  +"</td><td>" + item[item.length -1] + "</td></tr>";
                ...
                if (jsonTj.hasOwnProperty("dataXx") && jsonTj.dataXx != null) {
                    ...
                    if (jsonTj.dataXx != null) {
                        $.each(jsonTj.dataXx, function (index, item) {
                            if (index == 0) {//列名
                                _html_xx_thead = "<tr>";
                                $.each(item, function (i, data) {
                                    if (i <= item.length - 4) {
                                        _html_xx_thead += "<th>" + data + "</th>"
                                ...
                                _html_xx += "<tr class='link-stcd' _stcd='" + item[item.length - 3] + "' _stnm='" + item[0] + "' _sttp='" + item[item.length - 2] + "'><td>" + item[0] + "</td><td>" + item[1] + "</td><td>" + item[2] + "</td><td>" + item[3] + "</td><td>" + item[4] + "</td><td>" + item[5] + "</td><td>" + item[6] + "</td><td>"+ item[item.length - 1] + "</td></tr>";
                                if (item.Stcd == that.show_stcd) {
                                    existType = "xx";
                                    existIndex = i;
                ...
                if (jsonTj.hasOwnProperty("dataKzztz")) {
                    $("#linkTj_Kzztz_All").parent().show();
                    $("#linkTj_Kzztz_All").css({
                        "display": "block"
                    });
                } else {
                    $("#linkTj_Kzztz_All").parent().hide();
                    $("#linkTj_Kzztz_All").css({
                        "display": "none"
                    });
                };
            };
            if (that.show_type == "3") {//调度
                $("#tableHd_tj_dd thead").append(_html_hd_thead);
                $("#tableSk_tj_dd thead").append(_html_sk_thead);

                $("#tableHd_tj_dd tbody").append(_html_hd);
                $("#tableSk_tj_dd tbody").append(_html_sk);

                if (jsonTj.hasOwnProperty("dataHp")) {
                    $("#tableHp_tj_dd thead").append(_html_hp_thead);
                    $("#tableHp_tj_dd tbody").append(_html_hp);
                }
                if (jsonTj.hasOwnProperty("dataXx")) {
                    $("#tableXx_tj_dd thead").html("");
                    $("#tableXx_tj_dd tbody").html("");
                    $("#tableXx_tj_dd thead").append(_html_xx_thead);
                    $("#tableXx_tj_dd tbody").append(_html_xx);
                }
            }

            //排序
            if (that.show_type == "3") {
                $('#tableSk_tj_dd').DataTable({//水库
                    "destroy": true,
                    "retrieve": true,
                    "bStateSave": false,
                    // 是否允许检索
                ...
                if (jsonTj.hasOwnProperty("dataHp") && jsonTj.dataHp != null) {
                    $('#tableHp_tj_dd').DataTable({
                        "destroy": true,
                        "retrieve": true,
                        "bStateSave": false,
                        // 是否允许检索
            ...
            var $list;
            if (that.show_type == "3" && jsonTj.dataXx != null && jsonTj.hasOwnProperty("dataXx") && that.show_stnm != "洪泽湖") {
                existType = "xx";
            }
            if (existType == "hd") {
                $("#linkTj_Hd_All").click();
        ...
        var url = apiUrl_cloud + "api-realsituate/GetDisPatchInfoTj";
        $.ajax({
            type: 'post',
            contentType: "application/json; charset=utf-8",
            dataType: 'json',
            ...
            success: function (res) {
                ...
                if (json.code != 200) {
                    tools.loadinghide(false);
                    tools.show_message_error_const("获取调度后特征值统计信息失败");
    ...
    echart_line_gcx_real: function (stime, etime) {
        ...
        //初始化参数
        that.obj_fcch_gcx = {
            jjsw: panelConfig_all.MIN_NUM,
            bzsw: panelConfig_all.MIN_NUM,
            xxsw: panelConfig_all.MIN_NUM,
            zcsw: panelConfig_all.MIN_NUM,
            sjsw: panelConfig_all.MIN_NUM,
            jhsw: panelConfig_all.MIN_NUM,
            showJjsw: true,
            showBzsw: true,
            showXxsw: true,
            showZcsw: true
        ...
        that.obj_minMax_gcx = {
            sw: { min: panelConfig_all.MAX_NUM, max: panelConfig_all.MIN_NUM },
            ll: { min: panelConfig_all.MAX_NUM, max: panelConfig_all.MIN_NUM }
        };
        that.obj_data_gcx = {
            sssw: new Array(),
            sw_zx: new Array(),
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //特征水位获取
                if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {
                    that.obj_fcch_gcx.jjsw = json.Jjsw == "" ? panelConfig_all.MIN_NUM : Number(json.Jjsw);
                    that.obj_fcch_gcx.bzsw = json.Bzsw == "" ? panelConfig_all.MIN_NUM : Number(json.Bzsw);
                } else {
                    if (json.hasOwnProperty("Xxsw")) {
                        that.obj_fcch_gcx.xxsw = json.Xxsw == "" ? panelConfig_all.MIN_NUM : Number(json.Xxsw);
                    } else {
                        that.obj_fcch_gcx.zcsw = json.Zcxsw == "" ? panelConfig_all.MIN_NUM : Number(json.Zcxsw);
                    }
                    that.obj_fcch_gcx.sjsw = json.Sjsw == "" ? panelConfig_all.MIN_NUM : Number(json.Sjsw);
                    that.obj_fcch_gcx.jhsw = json.Jhsw == "" ? panelConfig_all.MIN_NUM : Number(json.Jhsw);
                }
                ...
                if (that.obj_data_gcx.sssw != null && that.obj_data_gcx.sssw.length > 0) {
                    ...
                    if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {
                        //警戒水位
                        if (that.obj_fcch_gcx.jjsw > panelConfig_all.MIN_NUM) {
                        ...
                        //保证水位
                        if (that.obj_fcch_gcx.bzsw > panelConfig_all.MIN_NUM) {
                        ...
                        //汛限水位
                        if (that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM) {
                        ...
                        //正常水位
                        if (that.obj_fcch_gcx.zcsw > panelConfig_all.MIN_NUM) {
                        ...
                        //设计水位
                        if (that.obj_fcch_gcx.sjsw > panelConfig_all.MIN_NUM) {
                        ...
                        //校核水位
                        if (that.obj_fcch_gcx.jhsw > panelConfig_all.MIN_NUM) {
                ...
                //警戒 保证  汛限是否勾选
                if (that.obj_fcch_gcx.jjsw > panelConfig_all.MIN_NUM && that.obj_minMax_gcx.sw.max > that.obj_fcch_gcx.jjsw) {
                    that.obj_fcch_gcx.showJjsw = true;
                } else {
                    that.obj_fcch_gcx.showJjsw = false;
                }
                if (that.obj_fcch_gcx.bzsw > panelConfig_all.MIN_NUM && that.obj_minMax_gcx.sw.max > that.obj_fcch_gcx.bzsw) {
                    that.obj_fcch_gcx.showBzsw = true;
                } else {
                    that.obj_fcch_gcx.showBzsw = false;
                }
                if (that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM && that.obj_minMax_gcx.sw.max > that.obj_fcch_gcx.xxsw) {
                    that.obj_fcch_gcx.showXxsw = true;
                } else {
                    that.obj_fcch_gcx.showXxsw = false;
                }
                if (that.obj_fcch_gcx.zcsw > panelConfig_all.MIN_NUM && that.obj_minMax_gcx.sw.max > that.obj_fcch_gcx.zcsw) {
    ...
    echart_line_gcx_for: function (stime, etime) {
        ...
        //初始化参数
        that.obj_fcch_gcx = {
            jjsw: panelConfig_all.MIN_NUM, bzsw: panelConfig_all.MIN_NUM,
            xxsw: panelConfig_all.MIN_NUM, zcsw: panelConfig_all.MIN_NUM, sjsw: panelConfig_all.MIN_NUM, jhsw: panelConfig_all.MIN_NUM,
            showJjsw: true, showBzsw: true, showXxsw: true, showSjsw: true, showJhsw: true
        };
        that.obj_minMax_gcx = {
            sw: { min: panelConfig_all.MAX_NUM, max: panelConfig_all.MIN_NUM },
            ll: { min: panelConfig_all.MAX_NUM, max: panelConfig_all.MIN_NUM },
            rain: { max: panelConfig_all.MIN_NUM }
        };
        that.obj_yb_graphic = {
                ...
                if (jsonFor.code == "0") {
                    ...
                    if (jsonFor.unitName != "") {
                        for (var i = 0; i < jsonFor.unitName.length; i++) {
                            unitName.push({ name: jsonFor.unitName[i], data: new Array() });
                        }
                    }
                    if (jsonFor.upStaName != "") {
                        for (var i = 0; i < jsonFor.upStaName.length; i++) {
                            upStaName.push({ name: jsonFor.upStaName[i], data: new Array() });
                        }
                    }
                    ...
                    try {
                        if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {//河道
                            that.obj_fcch_gcx.jjsw = (jsonFor.Jjsw == "" || jsonFor.Jjsw == "0") ? panelConfig_all.MIN_NUM : Number(jsonFor.Jjsw);
                            that.obj_fcch_gcx.bzsw = (jsonFor.Bzsw == "" || jsonFor.Bzsw == "0") ? panelConfig_all.MIN_NUM : Number(jsonFor.Bzsw);
                        } else {//水库、湖泊
                            that.obj_fcch_gcx.xxsw = (jsonFor.Xxsw == "" || jsonFor.Xxsw == "0") ? panelConfig_all.MIN_NUM : Number(jsonFor.Xxsw);
                            that.obj_fcch_gcx.sjsw = (jsonFor.Sjsw == "" || jsonFor.Sjsw == "0") ? panelConfig_all.MIN_NUM : Number(jsonFor.Sjsw);
                            that.obj_fcch_gcx.jhsw = (jsonFor.Jhsw == "" || jsonFor.Jhsw == "0") ? panelConfig_all.MIN_NUM : Number(jsonFor.Jhsw);
                        }
                        //var jsonTj = JSON.parse(that.ybTj_Data);
                        //if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {
                        //    $.each(jsonTj.dataHd, function (i, item) {
                        //        if (item.Stcd == that.show_stcd) {
                        //            that.obj_fcch_gcx.jjsw = (item.Jjsw == "" || item.Jjsw == "0") ? panelConfig_all.MIN_NUM : Number(item.Jjsw);
                        //            that.obj_fcch_gcx.bzsw = (item.Bzsw == "" || item.Bzsw == "0") ? panelConfig_all.MIN_NUM : Number(item.Bzsw);
                        //            return false;
                        //        }
                        //    });
                        //} else {
                        ...
                        //        if (item.Stcd == that.show_stcd) {
                        //            that.obj_fcch_gcx.xxsw = (item.Xxsw == "" || item.Xxsw == "0") ? panelConfig_all.MIN_NUM : Number(item.Xxsw);
                        //            that.obj_fcch_gcx.sjsw = (item.Sjsw == "" || item.Sjsw == "0") ? panelConfig_all.MIN_NUM : Number(item.Sjsw);
                        //            that.obj_fcch_gcx.jhsw = (item.Jhsw == "" || item.Jhsw == "0") ? panelConfig_all.MIN_NUM : Number(item.Jhsw);
                        //            return false;
                        //        }
                        //    });
                    ...
                    $.each(jsonFor.data, function (index, item) {
                        //雨量
                        that.obj_data_gcx.rain.push({ value: [item.TM, item.Pavg] });
                        ...
                        if (item.Zs != "" && item.Zs != "0") {
                            ...
                            that.obj_data_gcx_bk.ybsw.push({ value: [item.TM, item.Zs] });
                            ...
                            that.obj_data_gcx_bk.ybsw.push({ value: [item.TM, 0] });
                        ...
                        if (item.Qs != "" && item.Qs != "0") {
                            if (that.show_sttp != "RR") {
                                that.obj_data_gcx_bk.ll_in.push({ value: [item.TM, item.Qs] });
                        ...
                        if (unitName != null && unitName.length > 0) {
                            for (var i = 0; i < item.Qqj.length; i++) {
                                unitName[i].data.push({ value: [item.TM, item.Qqj[i]] });
                        ...
                        if (upStaName != null && upStaName.length > 0) {
                            for (var i = 0; i < item.Qhd.length; i++) {
                                upStaName[i].data.push({ value: [item.TM, item.Qhd[i]] });
                        ...
                        if (that.show_sttp == "ZZ") {
                          ...
                          */ } else if (that.show_sttp == "RR") {
                           ...
                           if (!((emptyAll_In || !emptyAll_In && moment(item.TM).diff(moment(emptyTM_In), "minutes", true) >= 0))) {
                               if (!(item.Qin == "" || item.Qin == "0")) {
                                   that.obj_data_gcx.ll_in.push({ value: [item.TM, item.Qin] });
                                   that.obj_data_gcx_bk.ll_in.push({ value: [item.TM, item.Qin] });
                                   that.obj_data_gcx_y_value.ll_in.push(item.Qin);
                                   that.obj_minMax_gcx.ll.min = Math.min(Number(item.Qin), that.obj_minMax_gcx.ll.min);
                                   that.obj_minMax_gcx.ll.max = Math.max(Number(item.Qin), that.obj_minMax_gcx.ll.max);
                               }
                               else {
                                   that.obj_data_gcx.ll_in.push({ value: [item.TM, 0] });
                                   that.obj_data_gcx_bk.ll_in.push({ value: [item.TM, 0] });
                                   that.obj_data_gcx_y_value.ll_in.push(0);
                                   that.obj_minMax_gcx.ll.min = Math.min(Number(0), that.obj_minMax_gcx.ll.min);
                                   that.obj_minMax_gcx.ll.max = Math.max(Number(0), that.obj_minMax_gcx.ll.max);
                           ...
                           /* else if (!((item.Qin == "" || item.Qin == "0") && (emptyAll_In || !emptyAll_In && moment(item.TM).diff(moment(emptyTM_In), "minutes", true) >= 0))) {
                                that.obj_data_gcx.ll_in.push({ value: [item.TM, item.Qin] });
                                that.obj_data_gcx_bk.ll_in.push({ value: [item.TM, item.Qin] });
                                that.obj_data_gcx_y_value.ll_in.push(item.Qin);
                                that.obj_minMax_gcx.ll.min = Math.min(Number(item.Qin), that.obj_minMax_gcx.ll.min);
                    ...
                    if (that.obj_data_gcx.rain != null && that.obj_data_gcx.rain.length > 0) {
                        ...
                        if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {
                            //警戒水位
                            if (that.obj_fcch_gcx.jjsw > panelConfig_all.MIN_NUM) {
                                that.obj_data_gcx.jjsw.push({ value: [that.obj_data_gcx.rain[0].value[0], that.obj_fcch_gcx.jjsw] });
                                that.obj_data_gcx.jjsw.push({ value: [maxTime, that.obj_fcch_gcx.jjsw] });
                            }
                            //保证水位
                            if (that.obj_fcch_gcx.bzsw > panelConfig_all.MIN_NUM) {
                                that.obj_data_gcx.bzsw.push({ value: [that.obj_data_gcx.rain[0].value[0], that.obj_fcch_gcx.bzsw] });
                                that.obj_data_gcx.bzsw.push({ value: [maxTime, that.obj_fcch_gcx.bzsw] });
                            }
                        } else if (that.show_sttp == "RR") {
                            //汛限水位
                            if (that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM) {
                                that.obj_data_gcx.xxsw.push({ value: [that.obj_data_gcx.rain[0].value[0], that.obj_fcch_gcx.xxsw] });
                                that.obj_data_gcx.xxsw.push({ value: [maxTime, that.obj_fcch_gcx.xxsw] });
                            }
                            //设计水位
                            if (that.obj_fcch_gcx.sjsw > panelConfig_all.MIN_NUM) {
                                that.obj_data_gcx.sjsw.push({ value: [that.obj_data_gcx.rain[0].value[0], that.obj_fcch_gcx.sjsw] });
                                that.obj_data_gcx.sjsw.push({ value: [maxTime, that.obj_fcch_gcx.sjsw] });
                            }
                            //校核水位
                            if (that.obj_fcch_gcx.jhsw > panelConfig_all.MIN_NUM) {
                                that.obj_data_gcx.jhsw.push({ value: [that.obj_data_gcx.rain[0].value[0], that.obj_fcch_gcx.jhsw] });
                                that.obj_data_gcx.jhsw.push({ value: [maxTime, that.obj_fcch_gcx.jhsw] });
                            }
                        }
                    }
                    ...
                    if (that.obj_fcch_gcx.sjsw > panelConfig_all.MIN_NUM && that.obj_minMax_gcx.sw.max > that.obj_fcch_gcx.sjsw) {
                        that.obj_fcch_gcx.showSjsw = true;
                    } else {
                        that.obj_fcch_gcx.showSjsw = false;
                    }
                    if (that.obj_fcch_gcx.jhsw > panelConfig_all.MIN_NUM && that.obj_minMax_gcx.sw.max > that.obj_fcch_gcx.jhsw) {
            ...
            success: function (res) {
                ...
                try {
                    if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {//河道、闸坝
                        $.each(jsonDd.statis, function (i, item) {
                            that.obj_fcch_gcx.jjsw = (item[4] == "" || item[4] == "0") ? panelConfig_all.MIN_NUM : Number(item[4]);
                            that.obj_fcch_gcx.bzsw = (item[5] == "" || item[5] == "0") ? panelConfig_all.MIN_NUM : Number(item[5]);
                            return false;
                        });
                    } else {
                        $.each(jsonDd.statis, function (i, item) {
                            that.obj_fcch_gcx.xxsw = (item[5] == "" || item[5] == "0") ? panelConfig_all.MIN_NUM : Number(item[5]);
                            that.obj_fcch_gcx.sjsw = (item[6] == "" || item[6] == "0") ? panelConfig_all.MIN_NUM : Number(item[6]);
                            that.obj_fcch_gcx.jhsw = (item[7] == "" || item[7] == "0") ? panelConfig_all.MIN_NUM : Number(item[7]);
                            return false;
                        });
                    };
                ...
                if ((jsonDd.upZBName != null) && (jsonDd.upZBName != undefined) && (jsonDd.upZBName != "")) {
                    ...
                    $.each(jsonDd.data, function (index, item) {
                        arrZ.push({ TM: item.TM, Z: item.Z });
                        arrQ.push({ TM: item.TM, Q: item.Q });
                        arrQin.push({ TM: item.TM, Q: item.Qin });
                        $.each(item.Updd, function (index_inner, item_inner) {
                            that.obj_data_gcx.upZBData[index_inner].push({ value: [item.TM, item_inner] });
                ...
                $.each(jsonDd.data, function (index, item) {
                    ...
                    if (item.Zdd != "" && item.Zdd != "0") {
                        that.obj_data_gcx.ddsw.push({ value: [item.TM, item.Zdd] });
                    ...
                    if (item.Qdd != "" && item.Qdd != "0") {
                        that.obj_data_gcx.ddll.push({ value: [item.TM, item.Qdd] });
                ...
                if (jsonDd.data != null && jsonDd.data.length > 0) {
                    ...
                    if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {
                        //警戒水位
                        if (that.obj_fcch_gcx.jjsw > panelConfig_all.MIN_NUM) {
                            that.obj_data_gcx.jjsw.push({ value: [_minTime, that.obj_fcch_gcx.jjsw] });
                            that.obj_data_gcx.jjsw.push({ value: [_maxTime, that.obj_fcch_gcx.jjsw] });
                        }
                        //保证水位
                        if (that.obj_fcch_gcx.bzsw > panelConfig_all.MIN_NUM) {
                            that.obj_data_gcx.bzsw.push({ value: [_minTime, that.obj_fcch_gcx.bzsw] });
                            that.obj_data_gcx.bzsw.push({ value: [_maxTime, that.obj_fcch_gcx.bzsw] });
                        }
                    } else if (that.show_sttp == "RR") {
                        //汛限水位
                        if (that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM) {
    ...
    echart_bar_rain: function (stime, etime) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var json = res.data;
                //解析数据
                var _max = panelConfig_all.MIN_NUM;
                $.each(json.data, function (index, item) {
                ...
                //获取最大雨量
                var axis_rain_max = _max > 0 ? Math.ceil((_max * (1 + panelConfig_all.MAX_DEPART))) : 1;
                var max_rain_ryl_sum = that.arr_ryl_sum[that.arr_ryl_sum.length - 1].value[1];
                var axis_rain_max_sum = max_rain_ryl_sum > 0 ? Math.ceil((max_rain_ryl_sum * (1 + panelConfig_all.MAX_DEPART))) : 1;
                //获取时间轴刻度
                ...
                /*  that.chartRyl.on('click', function (ob) {
                      if (ob.seriesType == "bar") {
                          $("#rainInfoBody_All").hide();
                          $("#rainInfoHourBody_All").show();
                          $("#btnReturnRyl_All").show();
 ...
 **/
    ...
    echart_line_dmxx: function () {
        ...
        //初始化参数
        that.obj_fcch_dmxx = {
            sw: panelConfig_all.MIN_NUM,
            sw_zx: panelConfig_all.MIN_NUM,
            jjsw: panelConfig_all.MIN_NUM,
            bzsw: panelConfig_all.MIN_NUM,
            xxsw: panelConfig_all.MIN_NUM,
            sjsw: panelConfig_all.MIN_NUM,
            ssw: panelConfig_all.MIN_NUM,
            sw_time: "",
            sw_position: "0"
        };
        that.obj_minMax_dmxx = {
            gc: { min: panelConfig_all.MAX_NUM, max: panelConfig_all.MIN_NUM }
        };
        that.obj_data_dmxx = {
            data: new Array(),
            sw: new Array(),
        ...
        //左高程  右高程--用于模拟断面
        var leftGz = panelConfig_all.MAX_NUM;
        var rightGz = panelConfig_all.MAX_NUM;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var isEmpty = false;
                if ((that.obj_data_dmxx.data == null || that.obj_data_dmxx.data == 0) && leftGz != panelConfig_all.MAX_NUM) {
                    isEmpty = true;
                ...
                if (isEmpty) {
                    ...
                    if (max_gc_dmxx_temp - min_gc_dmxx_temp > 0) {
                        that.obj_minMax_dmxx.gc.min = min_gc_dmxx_temp - (max_gc_dmxx_temp - min_gc_dmxx_temp) * panelConfig_all.MIN_DEPART / 2;
                        that.obj_minMax_dmxx.gc.max = max_gc_dmxx_temp + (max_gc_dmxx_temp - min_gc_dmxx_temp) * panelConfig_all.MAX_DEPART / 2;
                    } else {
                ...
                if (that.obj_minMax_dmxx.gc.min < 0) {
                    //更新水位数据
                    that.obj_fcch_dmxx.sw = (that.obj_fcch_dmxx.sw != "" && that.obj_fcch_dmxx.sw != panelConfig_all.MAX_NUM) ? (Number(that.obj_fcch_dmxx.sw) - that.obj_minMax_dmxx.gc.min) : that.obj_fcch_dmxx.sw;
                    that.obj_fcch_dmxx.sw_zx = (that.obj_fcch_dmxx.sw_zx != "" && that.obj_fcch_dmxx.sw_zx != panelConfig_all.MAX_NUM) ? (Number(that.obj_fcch_dmxx.sw_zx) - that.obj_minMax_dmxx.gc.min) : that.obj_fcch_dmxx.sw_zx;
                    that.obj_fcch_dmxx.jjsw = (that.obj_fcch_dmxx.jjsw != "" && that.obj_fcch_dmxx.jjsw != panelConfig_all.MAX_NUM) ? (Number(that.obj_fcch_dmxx.jjsw) - that.obj_minMax_dmxx.gc.min) : that.obj_fcch_dmxx.jjsw;
                    that.obj_fcch_dmxx.bzsw = (that.obj_fcch_dmxx.bzsw != "" && that.obj_fcch_dmxx.bzsw != panelConfig_all.MAX_NUM) ? (Number(that.obj_fcch_dmxx.bzsw) - that.obj_minMax_dmxx.gc.min) : that.obj_fcch_dmxx.bzsw;
                    that.obj_fcch_dmxx.xxsw = (that.obj_fcch_dmxx.xxsw != "" && that.obj_fcch_dmxx.xxsw != panelConfig_all.MAX_NUM) ? (Number(that.obj_fcch_dmxx.xxsw) - that.obj_minMax_dmxx.gc.min) : that.obj_fcch_dmxx.xxsw;
                    that.obj_fcch_dmxx.sjsw = (that.obj_fcch_dmxx.sjsw != "" && that.obj_fcch_dmxx.sjsw != panelConfig_all.MAX_NUM) ? (Number(that.obj_fcch_dmxx.sjsw) - that.obj_minMax_dmxx.gc.min) : that.obj_fcch_dmxx.sjsw;
                    that.obj_fcch_dmxx.ssw = (that.obj_fcch_dmxx.ssw != "" && that.obj_fcch_dmxx.ssw != panelConfig_all.MAX_NUM) ? (Number(that.obj_fcch_dmxx.ssw) - that.obj_minMax_dmxx.gc.min) : that.obj_fcch_dmxx.ssw;
    ...
    setPanelSize: function (width, height, isResize, isInit) {
        ...
        var tjtableHeight = $("#tj_tbody_all").height();

        //获取真实宽度
        var _width = width - 20;
        $("#contProLine_All").width(_width - 200);
        var _height = height - 32 - 33 - 15;
        if ((that.show_type == "2" || that.show_type == "3") && that.show_all) {
            //_height = _height - panelConfig_all.table_height_yb;
            //$(this.parentId + " .tj_body_all").width(_width - 200);
            //$(this.parentId + " .tableHd," + this.parentId + " .tableHp," + this.parentId + " .tableXx").parent().height(panelConfig_all.table_height_yb - 33 - 33 - 10);
        ...
        if (that.show_type == "1" || ((that.show_type == "2" || that.show_type == "3") && !that.show_all)) {
            ...
            if (that.show_type == "3") {
                ...
                $("#proLineBody_All").width(_width - 575 - 200).height(_height - (that.show_type != "1" ? 105 : 0) - (($(".ybsw-modify").is(":visible")) ? 45 : 0)); //2019-10-8 modify by chl
                $("#contProLine_All .content-left").css("margin-right", 575);
                $("#contProLine_All .content-left").css("margin-top", -28);
                $("#contProLine_All .sidebar-right").width(550);

            } else {

                $("#proLineBody_All").width(_width - 425 - 2 - 200).height(_height - (that.show_type != "1" ? 105 : 0) - (($(".ybsw-modify").is(":visible")) ? 45 : 0)); //2019-10-8 modify by chl
                $("#contProLine_All .content-left").css("margin-right", 425);
                $("#contProLine_All .content-left").css("margin-top", 0);
                $("#contProLine_All .sidebar-right").width(400);
                /*
                  var isshow_right = $("#contProLine_All .sidebar-right").width() == 0 ? false : true;
                $("#proLineBody_All").width(_width - (isshow_right==true?425:0) - 200).height(_height - (that.show_type != "1" ? 105 : 0) - (($(".ybsw-modify").is(":visible")) ? 45 : 0)); //2019-10-8 modify by chl
                $("#contProLine_All .content-left").css("margin-right", (isshow_right == true ? 425 : 25));
                $("#contProLine_All .content-left").css("margin-top", 0);
                $("#contProLine_All .sidebar-right").width((isshow_right == true ? 400 : 0));
                 */


            }
            $("#contProLine_All .tableSqxx").parent().height(_height - (that.show_type != "1" ? 105 : 0) - 48);
            $("#contProLine_All .ybsq-table-header").css({
                "width": "100%",
                "padding-right": tools.scroll_default_width + "px"
            });
        } else {
            isshowall = true;
            $("#proLineBody_All").css("margin-top", 0);
            var tjtableHeight = $("#tj_tbody_all").height();
            $("#proLineBody_All").width(_width).height(_height - tjtableHeight - 35);
            //$("#proLineBody_All").width(_width).height(_height - (($(".ybsw-modify").is(":visible")) ? 45 : 0));
        }
        ...
        if (!isshowall) {
            if (isInit) {
                $("#contProLine_All .content-left").show();
                $("#proLineBody_All").show();
                $("#contProLine_All .sidebar-right").show();
            }
            else {
                //判断当前状态，是两个都有，还是单个
                var v_left = $("#proLineBody_All").is(':visible');
                var v_right = $("#contProLine_All .sidebar-right").is(':visible');
                if ((v_left) && (v_right)) {
                    $("#contProLine_All .sidebar-right").show();
                    $("#proLineDd").show();
                    $("#contProLine_All .content-left").show();
                    ...
                    $("#proLineBody_All").width(that.panel_actual_width - 200 - 20 - 25 - 2).height(_height - (that.show_type != "1" ? 105 : 0) - (($(".ybsw-modify").is(":visible")) ? 45 : 0)); //2019-10-8 modify by chl
                    $("#contProLine_All .content-left").css("margin-right", 25);
                    $("#contProLine_All .sidebar-right").width(0);
                }
                else if (v_right) {
                    $("#proLineBody_All").width(0).height(_height - (that.show_type != "1" ? 105 : 0) - (($(".ybsw-modify").is(":visible")) ? 45 : 0)); //2019-10-8 modify by chl
                    $("#contProLine_All .content-left").css("margin-right", that.panel_actual_width - 200 - 20);

                    $("#contProLine_All .sidebar-right").width(that.panel_actual_width - 200 - 20 - 25 - 2);
                }
        ...
        //降雨信息
        $("#rainInfoBody_All").width(_width - 10).height(_height - tjtableHeight - 35); //2019-10-8 modify by chl
        $("#rainInfoHourBody_All").width(_width - 10).height(_height - 10);
        if (isResize) {
        ...
        //断面信息
        $("#dmInfoBody_All").width(_width).height(_height - tjtableHeight - 35); //2019-10-8 modify by chl
        if (isResize) {
            this.chartDm.resize();
        }
    ...
    create_table_sqxx_real: function (json) {
        ...
        //    水库：时间、水位、入库流量、出库流量
        //先清空数据
        $("#contProLine_All .ybsq-table-header table colgroup col").remove();
        $("#contProLine_All .ybsq-table-header table thead tr").remove();
        $("#contProLine_All .tableSqxx colgroup col").remove();
        $("#contProLine_All .tableSqxx tbody tr").remove();
        var _html_col = "";
        var _html_th = "";
        if (that.show_sttp == "ZZ") {
        ...
        $("#contProLine_All .ybsq-table-header table colgroup").append(_html_col);
        $("#contProLine_All .tableSqxx colgroup").append(_html_col);
        $("#contProLine_All .ybsq-table-header table thead").append(_html_th);
        ...
        $("#contProLine_All .tableSqxx tbody").append(_html_body);
    },
    /**
     * 生成表格-预报水情
     **/
    create_table_sqxx: function (json) {
        ...
        $("#contProLine_All .tableSqxx tbody").append(_html_body);
        //  $("#contProLine_All .tableSqxx .edit_input input").removeAttr("readonly");
        //生成该站统计信息
        try {
            $("#contProLine_All .table_ybtj thead tr").remove();
            $("#contProLine_All .table_ybtj tbody tr").remove();
            var jsonTj = JSON.parse(that.ybTj_Data);
            if (that.show_sttp == "ZZ" || that.show_sttp == "DD") {
                $("#contProLine_All .table_ybtj thead").append("<tr><th>站名</th><th>洪峰水位</th><th>洪峰水位时间</th><th>洪峰流量</th><th>警戒水位</th><th>保证水位</th><th>保证流量</th></tr>");
                var isExist = false;
                $.each(jsonTj.dataHd, function (i, item) {
                    if (item.Stcd == that.show_stcd) {
                        isExist = true;
                        ...
                        $("#contProLine_All .table_ybtj tbody").append("<tr><td>" + item.Stnm + "</td><td>" + item.Hfsw + "</td><td>" + item.Hwtm + "</td><td>" + item.Hfll + "</td><td>" + item.Jjsw + "</td><td>" + item.Bzsw + "</td><td>" + item.Bzll + "</td></tr>");
                        return false;
                    }
                });
                if (!isExist) {
                    $("#contProLine_All .table_ybtj tbody").append("<tr><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>");
                }
            } else {
                //湖泊站
                if (that.show_stcd == "51201601" || that.show_stcd == "51203401" || that.show_stcd == "50916500" || that.show_stcd == "51107801" || that.show_stcd == "51003650" || that.show_stcd == "51003800") {
                    $("#contProLine_All .table_ybtj thead").append("<tr><th>站名</th><th>当前水位</th><th>现状下泄流量</th><th>预报最高水位</th><th>超汛限水位</th><th>预报总来水量</th><th>汛限水位</th><th>设计洪水位</th><th>设计库容</th><th>校核洪水位</th><th>总库容</th><th>历史最高水位</th></tr>");
                    var isExist = false;
                    $.each(jsonTj.dataHp, function (i, item) {
                        if (item.Stcd == that.show_stcd) {
                            isExist = true;
                            ...
                            $("#contProLine_All .table_ybtj tbody").append("<tr><td>" + item.Stnm + "</td><td>" + item.Z + "</td><td>" + item.Otq + "</td><td>" + item.MaxZ + "</td><td>" + item.Cxx + "</td><td>" + item.Yblsl + "</td><td>" + item.Xxsw + "</td><td>" + item.Sjsw + "</td><td>" + item.Sjw + "</td><td>" + item.Jhsw + "</td><td>" + item.W + "</td><td>" + item.Lszg + "</td></tr>");
                            return false;
                        }
                    });
                    if (!isExist) {
                        $("#contProLine_All .table_ybtj tbody").append("<tr><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>");
                    }
                } else {
                    //水库站
                    $("#contProLine_All .table_ybtj thead").append("<tr><th>站名</th><th>当前水位</th><th>当前出库流量</th><th>预报最高水位</th><th>超汛限水位</th><th>汛限水位</th><th>设计水位</th><th>校核水位</th><th>全陪高程</th><th>移民高程</th><th>历史最高水位</th></tr>");
                    var isExist = false;
                    $.each(jsonTj.dataSk, function (i, item) {
                        if (item.Stcd == that.show_stcd) {
                            isExist = true;
                            ...
                            item.Lszg = item.Lszg == "0" ? "" : item.Lszg;
                            $("#contProLine_All .table_ybtj tbody").append("<tr><td>" + item.Stnm + "</td><td>" + item.Z + "</td><td>" + item.Otq + "</td><td>" + item.MaxZ + "</td><td>" + item.Cxx + "</td><td>" + item.Xxsw + "</td><td>" + item.Sjsw + "</td><td>" + item.Jhsw + "</td><td>" + item.Qpgc + "</td><td>" + item.Ymgc + "</td><td>" + item.Lszg + "</td></tr>");
                            return false;
                        }
                    });
                    if (!isExist) {
                        $("#contProLine_All .table_ybtj tbody").append("<tr><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>");
                    }
                }
            }
        } catch (e) {
...
**/
        ...
        that.chartGcx.on("legendselectchanged", function (params) {
            ...
            if (that.show_sttp == "RR") {
                if (params.name == "汛限水位" || params.name == "正常蓄水位" || params.name == "设计水位" || params.name == "校核水位") {
                    var tempSw = that.obj_minMax_gcx.sw.max;
                    if (params.selected.汛限水位 && that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM) {
                        tempSw = Math.max(tempSw, that.obj_fcch_gcx.xxsw);
                        that.obj_minMax_gcx.sw.min = Math.min(that.obj_fcch_gcx.xxsw, that.obj_minMax_gcx.sw.min);
                    }
                    if (params.selected.正常蓄水位 && that.obj_fcch_gcx.zcsw > panelConfig_all.MIN_NUM) {
                    ...
                    if (params.selected.设计水位 && that.obj_fcch_gcx.sjsw > panelConfig_all.MIN_NUM) {
                        tempSw = Math.max(tempSw, that.obj_fcch_gcx.sjsw);
                        that.obj_minMax_gcx.sw.min = Math.min(that.obj_fcch_gcx.sjsw, that.obj_minMax_gcx.sw.min);
                    }
                    if (params.selected.校核水位 && that.obj_fcch_gcx.jhsw > panelConfig_all.MIN_NUM) {
                        tempSw = Math.max(tempSw, that.obj_fcch_gcx.jhsw);
                        that.obj_minMax_gcx.sw.min = Math.min(that.obj_fcch_gcx.jhsw, that.obj_minMax_gcx.sw.min);
                    }
                    var arrSw = new Array();
                    if (Math.abs(tempSw) == Math.abs(panelConfig_all.MAX_NUM) || Math.abs(that.obj_minMax_gcx.sw.min) == Math.abs(panelConfig_all.MIN_NUM)) {
                        arrSw = [1, 0];
                    } else {
                ...
                if (params.name == "警戒水位" || params.name == "保证水位") {
                    var tempSw = that.obj_minMax_gcx.sw.max;
                    if (params.selected.警戒水位 && that.obj_fcch_gcx.jjsw > panelConfig_all.MIN_NUM) {
                        tempSw = Math.max(tempSw, that.obj_fcch_gcx.jjsw);
                        that.obj_minMax_gcx.sw.min = Math.min(that.obj_fcch_gcx.jjsw, that.obj_minMax_gcx.sw.min);
                    }
                    if (params.selected.保证水位 && that.obj_fcch_gcx.bzsw > panelConfig_all.MIN_NUM) {
                        tempSw = Math.max(tempSw, that.obj_fcch_gcx.bzsw);
                        that.obj_minMax_gcx.sw.min = Math.min(that.obj_fcch_gcx.bzsw, that.obj_minMax_gcx.sw.min);
                    }
                    var arrSw = new Array();
    ...
    get_axis_sw_max_min_gcx: function () {
        var that = this;
        var axis_sw_min_temp = panelConfig_all.MAX_NUM;
        var axis_sw_max_temp = panelConfig_all.MIN_NUM;
        var axis_sw_min = panelConfig_all.MAX_NUM;
        var axis_sw_max = panelConfig_all.MIN_NUM;
        ...
        if (axis_sw_max_temp - axis_sw_min_temp > 0) {
            axis_sw_min = axis_sw_min_temp - (axis_sw_max_temp - axis_sw_min_temp) * panelConfig_all.MIN_DEPART;
            axis_sw_max = axis_sw_max_temp + (axis_sw_max_temp - axis_sw_min_temp) * panelConfig_all.MAX_DEPART;
        } else {
            if (axis_sw_min_temp == panelConfig_all.MAX_NUM) {
                axis_sw_min = 0;
                axis_sw_max = 1;
            } else {
    ...
    get_ll_max_min: function (axis_ll_max_temp, axis_ll_min_temp) {
        var that = this;
        var axis_ll_min = panelConfig_all.MAX_NUM;
        var axis_ll_max = panelConfig_all.MIN_NUM;

        //获取流量最大 最小
        if (axis_ll_max_temp - axis_ll_min_temp > 0) {
            axis_ll_min = axis_ll_min_temp - (axis_ll_max_temp - axis_ll_min_temp) * panelConfig_all.MIN_DEPART;
            axis_ll_max = axis_ll_max_temp + (axis_ll_max_temp - axis_ll_min_temp) * panelConfig_all.MAX_DEPART;
        } else {
            if (axis_ll_min_temp == panelConfig_all.MAX_NUM) {
                axis_ll_min = 0;
                axis_ll_max = 1;
            } else {
    ...
    get_axis_ll_max_min_gcx: function () {
        var that = this;
        var axis_ll_min_temp = panelConfig_all.MAX_NUM;
        var axis_ll_max_temp = panelConfig_all.MIN_NUM;
        var axis_ll_min = panelConfig_all.MAX_NUM;
        var axis_ll_max = panelConfig_all.MIN_NUM;
 ...
 **/
        ...
        var option = {
            ...
            xAxis: {
                ...
                axisLine: {
                    onZero: false,
                    lineStyle: {
                        color: panelConfig_all.ECHART_COLOR_AXIS_X
                    }
                },
                interval: intervalX,
            ...
            yAxis: [{
                ...
                axisLine: {
                    onZero: false,
                    lineStyle: {
                        color: panelConfig_all.ECHART_COLOR_RAIN
                    }
                },
                splitLine: {
                    show: false
                ...
                axisLine: {
                    onZero: false,
                    lineStyle: {
                        color: panelConfig_all.ECHART_COLOR_RAIN_SUM
                    }
                },
                splitLine: {
                    show: false
            ...
            series: [{
                ...
                label: {
                    show: true,
                    position: "top",
                    color: panelConfig_all.ECHART_COLOR_RAIN_OUT,
                    fontSize: 10,
                    fontWeight: 600,
                    formatter: function (ob) {
                ...
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_RAIN_BAR,
                    shadowColor: panelConfig_all.ECHART_COLOR_SHADOW,
                    shadowBlur: 5
                },
                barWidth: "95%",
            ...
            backgroundColor: panelConfig_all.ECHART_COLOR_BACKGROUND
        };
        option = tools.initChartlegendIcon(option);
        return option;
    },
        ...
        var option = {
            ...
            xAxis: {
                ...
                splitLine: {
                    show: true,
                    lineStyle: {
                        color: [panelConfig_all.ECHART_COLOR_GRID]
                    }
                }
            },
            yAxis: [{
                ...
                axisLine: {
                    onZero: false,
                    lineStyle: {
                        color: panelConfig_all.ECHART_COLOR_SW
                    }
                },
                axisLabel: {
                    formatter: function (value, index) {
                ...
                axisLine: {
                    onZero: false,
                    lineStyle: {
                        color: panelConfig_all.ECHART_COLOR_LL
                    }
                },
                axisLabel: {
                    formatter: function (value, index) {
            ...
            series: [{
                ...
                showSymbol: false,
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_JJSW
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    symbolOffset: [60, 0], //偏移值X Y
                    label: {
                        backgroundColor: panelConfig_all.ECHART_COLOR_JJSW,
                        padding: [5, 10, 5, 10],
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
                ...
                showSymbol: false,
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_BZSW
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    symbolOffset: [140, 0], //偏移值X Y
                    label: {
                        backgroundColor: panelConfig_all.ECHART_COLOR_BZSW,
                        padding: [5, 10, 5, 10],
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
    ...
    get_option_gcx_dd_real: function (axis_sw_max, axis_sw_min, axis_ll_max, axis_ll_min, intervalX, strFormatX) {
        ...
        var option = {
            ...
            //     yAxisIndex: 0,
            //     itemStyle: {
            //         color: panelConfig_all.ECHART_COLOR_ZXSW
            //     },
            //     showSymbol: false,
    ...
    get_option_gcx_rr_real: function (axis_sw_max, axis_sw_min, axis_ll_max, axis_ll_min, intervalX, strFormatX) {
        ...
        var option = {
            legend: {
                ...
                itemWidth: 20,
                itemHeight: 8,
                data: ["水位", "入库流量", "出库流量", that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM ? "汛限水位" : "正常蓄水位", "设计水位", "校核水位"],
                selected: {
            ...
            series: [{
                ...
                symbol: "circle",
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_LL_DNTQ
                },
                showSymbol: false,
                connectNulls: true,   //连接空数据
                smooth: true,     //平滑显示曲线
                ...
                name: that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM ? "汛限水位" : "正常蓄水位",
                type: 'line',
                yAxisIndex: 0,
                symbol: "circle",
                showSymbol: false,
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_XXSW
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    symbolOffset: [60, 0], //偏移值X Y
                    label: {
                        backgroundColor: panelConfig_all.ECHART_COLOR_XXSW,
                        padding: [5, 10, 5, 10],
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
                                return that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM ? "汛限水位：" + that.obj_fcch_gcx.xxsw : "正常蓄水位：" + that.obj_fcch_gcx.zcsw;
                            } else {
                                return "";
                            }
                        }
                ...
                data: that.obj_fcch_gcx.xxsw > panelConfig_all.MIN_NUM ? that.obj_data_gcx.xxsw : that.obj_data_gcx.zcsw
            }, {
                name: '设计水位',
                type: 'line',
                yAxisIndex: 0,
                ...
                showSymbol: false,
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_SJSW
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    symbolOffset: [140, 0], //偏移值X Y
                    label: {
                        backgroundColor: panelConfig_all.ECHART_COLOR_SJSW,
                        padding: [5, 10, 5, 10],
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
                ...
                showSymbol: false,
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_JHSW
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    symbolOffset: [210, 0], //偏移值X Y
                    label: {
                        backgroundColor: panelConfig_all.ECHART_COLOR_JHSW,
                        padding: [5, 10, 5, 10],
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
    ...
    get_option_gcx_zz_for: function (axis_sw_max, axis_sw_min, axis_ll_max, axis_ll_min, intervalX, strFormatX) {
        ...
        var option = {
            ...
            yAxis: [{
                ...
                inverse: true,
                min: 0,
                max: that.obj_minMax_gcx.rain.max == panelConfig_all.MIN_NUM ? 1 : parseInt(that.obj_minMax_gcx.rain.max * 8),
                axisLine: {
                    lineStyle: {
                        color: panelConfig_all.ECHART_COLOR_RAIN
                    }
    ...
    onSWPointDragend: function (dataIndex, that, dx, dy) {
        ...
        $("#contProLine_All .tableSqxx tbody").find("tr").each(function () {
    ...
    get_option_gcx_zz_dd: function (axis_sw_max, axis_sw_min, axis_ll_max, axis_ll_min, intervalX, strFormatX) {
        ...
        arr_series.push({
            ...
            hoverAnimation: false,
            itemStyle: {
                color: panelConfig_all.ECHART_COLOR_DDSW
            },
            lineStyle: {
                type: "dashed"
            },
        ...
        arr_series.push({
            ...
            hoverAnimation: false,
            itemStyle: {
                color: panelConfig_all.ECHART_COLOR_DDLL
            },
            lineStyle: {
                type: "dashed",
            },
    ...
    get_option_dmxx_zz: function (axis_x_min, axis_x_max) {
        ...
        var option = {
            ...
            series: [{
                ...
                areaStyle: {
                    color: panelConfig_all.ECHART_COLOR_DM_SW,
                    opacity: 1
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    ...
                    label: {
                        show: true,
                        color: "#fff",
                        backgroundColor: panelConfig_all.ECHART_COLOR_SW,
                        padding: [2, 10, 3, 10],
                        lineHeight: 16,
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
                ...
                z: 4,
                lineStyle: {
                    color: panelConfig_all.ECHART_COLOR_DM_LINE,
                    width: 2
                },
                areaStyle: {
                    color: panelConfig_all.ECHART_COLOR_DM,
                    opacity: 1
                },
    ...
    get_option_dmxx_dd: function (axis_x_min, axis_x_max) {
        ...
        var option = {
            ...
            series: [{
                ...
                areaStyle: {
                    color: panelConfig_all.ECHART_COLOR_DM_ZXSW,
                    opacity: 1
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    ...
                    label: {
                        show: true,
                        color: "#fff",
                        backgroundColor: panelConfig_all.ECHART_COLOR_ZXSW,
                        padding: [5, 10, 5, 10],
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
    ...
    get_option_dmxx_rr: function (axis_x_min, axis_x_max) {
        ...
        var option = {
            ...
            series: [{
                ...
                z: 5,
                itemStyle: {
                    color: panelConfig_all.ECHART_COLOR_SSW
                },
                markPoint: {
                    symbolSize: 1,    //设置为0时 不显示标志
                    symbolOffset: [60, 0], //偏移值X Y
                    ...
                    label: {
                        show: true,
                        color: "#fff",
                        backgroundColor: panelConfig_all.ECHART_COLOR_SSW,
                        padding: [5, 10, 5, 10],
                        formatter: function (ob) {
                            if (ob.dataIndex == 0) {
    ...
    get_min_max: function (arr) {
        var min = panelConfig_all.MAX_NUM;
        var max = panelConfig_all.MIN_NUM;
        if (arr == null || arr.length == 0) {
