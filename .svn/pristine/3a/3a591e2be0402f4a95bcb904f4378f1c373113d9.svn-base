//# sourceURL=sqyj_report.js
//报表默认宽高
var default_sqyj_width_report = 1300;
...
function getForeWsReport(model) {
...
function create_sqyj_table_hd(json, model) {
    ...
    $("#stationNum").html("");
    modelName = json.basinReportInfoList[0].riverReportInfoList[0].modelName
    $("#panel_sqyj .modal-title").html(modelName + "水情预警");
    //单流域 
    ...
    $.each(riverStation, function (i, data) {
        ...
        if (i == "wrzNum") {
            ...
            stationHtml += "河道超警戒：:" + font_start + data + font_end + "个；"
        }
        if (i == "grzNum") {
            if (data > 0) {
            ...
            stationHtml += "河道超保证：:" + font_start + data + font_end + "个；"
        }
        if (i == "obhtzNum") {
            if (data > 0) {
            ...
            stationHtml += "河道超历史：:" + font_start + data + font_end + "个；"
        }
    })
    $.each(reservoirStation, function (i, data) {
        ...
        if (i == "fsltdzNum") {
            ...
            stationHtml += "水库超汛限：:" + font_start + data + font_end + "个；"
        }
        if (i == "normzNum") {
            if (data > 0) {
                font_start = "<font color='blue' style='font-weight:900'>"
            ...
            stationHtml += "水库超正常：:" + font_start + data + font_end + "个；"
        }
        if (i == "dsflzNum") {
            if (data > 0) {
                font_start = "<font color='red' style='font-weight:900'>"
            ...
            stationHtml += "水库超设计：:" + font_start + data + font_end + "个；"
        }
        if (i == "ckflzNum") {
            if (data > 0) {
                font_start = "<font color='purple' style='font-weight:900'>"
            ...
            stationHtml += "水库超校核：:" + font_start + data + font_end + "个；"
        }
    })
    stationHtml += "</span>"
    $("#stationNum").append(stationHtml);
    //表头
...
function show_ybgcx(stcd, stnm, sttp) {
    ...
    GetTreeMenuData();
    tools_panel.panelShow(stcd, stnm, searchEtime, sttp, searchStime);
}